...
if [ "`flask fab list-users | grep 'username:admin'`" == "" ]; then
	flask fab create-admin --username ${SUPERSET_ADM_USR:-admin} \
		--firstname ${SUPERSET_ADM_FIRSTNAME:-admin} \
		--lastname ${SUPERSET_ADM_LASTNAME:-user} \
		--email ${SUPERSET_ADM_EMAIL:-admin@localhost} \
		--password ${SUPERSET_ADM_PWD:-admin}
fi
